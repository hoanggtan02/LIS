...
const Cart = {
  ...
  add(product, color, size, qty = 1) {
    ...
    else items.push({ key, product_id: product.id, name: product.name, price: product.display_price, image: product.images?.[0] || '', color, size, quantity: qty });
    this.save(items);
    Toast.show('Đã thêm vào giỏ hàng 🛒', 'success');
  },
  remove(key)      { this.save(this.get().filter(i => i.key !== key)); },
  update(key, qty) { if (qty <= 0) return this.remove(key); const items = this.get(); const item = items.find(i => i.key === key); if (item) { item.quantity = qty; this.save(items); } },
